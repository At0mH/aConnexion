...
local connectMenu = RageUI.CreateMenu("Bienvenue", "~r~NOM DE VOTRE SERVEUR")
    ...
    if not isMenuOpen then
        ...
        while isMenuOpen do
            RageUI.IsVisible(connectMenu, function()
                RageUI.Button("Appuyer sur ~b~[ENTRER]~s~ pour vous ~g~connecter", nil, {}, not connect, {
                    onSelected = function()
                        connect = true
                    end
                })
                ...
                if connect then
                    ...
                    if chargement < 5.0 then
                        chargement = chargement + 5.002
                    else 
                        chargement = 0
                    end
                    ...
                    if chargement >= 1.0 then
                        ...
                        ESX.ShowAdvancedNotification("Bienvenue", "~g~Information", "Bienvenue sur ~g~NOM DE VOTRE SERVEUR", 'CHAR_SOCIAL_CLUB', 1)
                        DisplayRadar(true)
                    end
                end
            end)
...
function StartAnimConnect()
    ...
    SetCamCoord(CameraConnect, -1413.84, -1004.47, 52.717)
